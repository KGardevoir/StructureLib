...
dlist*
make_dlist(int a[], size_t s, list_tspec* type){
	long i = 0;
	dlist* newl = NULL;
	for(; i < s; i++){
...
main(int argc, char** argv){
	list_tspec list_type = {malloc, free, NULL, compare_ints, compare_ints, NULL, NULL};
	int x1[] = { 1, 3, 5, 6, 9, 10, 13 };
	int x2[] = { 7, 6, 5, 4, 3, 2, 1, 8, 11, 14, 9, 10 };
	int x3[] = { 3, 4, 7, 8, 11, 12, 14, 15, 16, 17, 18 };
	printf("List  1---------------------------\n");
	dlist *l1 = print_list(make_dlist(x1, sizeof(x1)/sizeof(int), &list_type));
	printf("List  2---------------------------\n");
	dlist *l2 = print_list(make_dlist(x2, sizeof(x2)/sizeof(int), &list_type));
	printf("List  3---------------------------\n");
	dlist *l3 = print_list(make_dlist(x3, sizeof(x3)/sizeof(int), &list_type));
	//printf("Test  1---------------------------\n");
