...
void
SLL_PREFIX-{}-printList(SLL_NAME* head, TSPEC_NAME* type){
	SLL_NAME *run;
	size_t i = 0;
	if(!head){
		printf("Empty List\n");
...
SLL_NAME*
SLL_PREFIX-{}-pushList(SLL_NAME* he, void* buf, BOOLEAN copy, TSPEC_NAME *type){
	SLL_NAME *lm;
...
SLL_NAME*
SLL_PREFIX-{}-queueList(SLL_NAME* he, void* buf, BOOLEAN copy, TSPEC_NAME* type){
	SLL_NAME *lm, *he2;
	lm = (SLL_NAME*)type->adalloc(sizeof(SLL_NAME));
	if(copy){
		lm->data = type->deep_copy(buf);
...
SLL_PREFIX-{}-addOrdered(SLL_NAME* he, void* buf, BOOLEAN deep_copy, BOOLEAN overwrite, TSPEC_NAME* type){
	if(!type->compar) return SLL_PREFIX-{}-pushList(he, buf, deep_copy, type); //add to front if compar is null
...
SLL_NAME*
SLL_PREFIX-{}-copyList(SLL_NAME* src, BOOLEAN deep_copy, TSPEC_NAME* type){
	SLL_NAME *the_list, *list_start, *runner = src;
	void* new_data = runner->data;
	if(deep_copy) new_data = type->deep_copy(new_data);
	the_list = list_start = SLL_PREFIX-{}-pushList(NULL, new_data, 0, FALSE);
	runner = runner->next;
	for(; runner != NULL; runner = runner->next){
		void* new_data = runner->data;
		if(deep_copy) new_data = type->deep_copy(new_data);
		the_list = SLL_PREFIX-{}-queueList(the_list, new_data, 0, FALSE)->next;
	}
	return list_start;
}
...
void
SLL_PREFIX-{}-clearList(SLL_NAME *head, BOOLEAN free_data, TSPEC_NAME* type){
...
SLL_NAME*
SLL_PREFIX-{}-dequeueList(SLL_NAME *head, void** data, BOOLEAN free_data, TSPEC_NAME* type){
	if(!head) return NULL;
	SLL_NAME *run = head, *p = head;
	if(!run->next){
...
SLL_NAME*
SLL_PREFIX-{}-popList(SLL_NAME *head, void** data, BOOLEAN free_data, TSPEC_NAME* type){
	SLL_NAME *run = head;
	if(!head) return head;
	run = head;
	head = head->next;
...
SLL_NAME*
SLL_PREFIX-{}-removeElementKey(SLL_NAME *head, void** data, void* key, BOOLEAN ordered, BOOLEAN free_data, TSPEC_NAME* type){
...
ifdef(-{DCLL}-,-{dnl
DCLL_PREFIX-{}-linkedList*
SLL_PREFIX-{}-linkedList_to_double(SLL_NAME *head, BOOLEAN deep, TSPEC_NAME* type){
	SLL_NAME* p = head;
	DCLL_PREFIX-{}-linkedList* d = NULL;
	for(; p; p = p->next){
		d = DCLL_PREFIX-{}-queueList(d, NULL, p->data, deep, type);
	}
	return d;
}
}-)dnl
...
SLL_NAME*
SLL_PREFIX-{}-mergeLists(SLL_NAME* dst, SLL_NAME* src, TSPEC_NAME* type){
	if(type->compar){//merge sorted, assume both lists are already sorted
		...
		do{
			...
			if(!l2_done){
				...
				SLL_PREFIX-{}-splitList(runsrc, runsrc_end);
			}
			//ldp("Merging", runsrc, NULL);
			SLL_PREFIX-{}-concatLists(rundst, runsrc);
		...
		return SLL_PREFIX-{}-concatLists(dst, src);
	}
}

SLL_NAME*
SLL_PREFIX-{}-mergeSort(SLL_NAME* head, TSPEC_NAME* type){//this can be faster, using "natural" mergesort
	...
	if(left != right){
		...
		*/
		...
		SLL_PREFIX-{}-splitList(left, right);
		left = SLL_PREFIX-{}-mergeSort(left, type);
		right = SLL_PREFIX-{}-mergeSort(right, type);
		return SLL_PREFIX-{}-mergeLists(left, right, type);
	}
	return head;
}

SLL_NAME*
SLL_PREFIX-{}-splitList(SLL_NAME* h1, SLL_NAME* h2){
	return h2;
}

SLL_NAME*
SLL_PREFIX-{}-concatLists(SLL_NAME *dst, SLL_NAME* src){
	SLL_NAME *t = dst;
...
void*
SLL_PREFIX-{}-findInList(SLL_NAME *head, void* key, BOOLEAN ordered, TSPEC_NAME *type){
	if(!head) return NULL;
SLL_NAME* p = head;
